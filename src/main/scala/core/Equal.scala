...
object Equal {
  def instance[A](f: (A, A) => Boolean): Equal[A] = new Equal[A] {
    def isEqual(lhs: A, rhs: A): Boolean = f(lhs, rhs)
  }
  ...
  implicit def eqList[A](implicit EA: Equal[A]): Equal[List[A]] =
    instance((x, y) =>
      x.size == y.size && {
        x.zip(y).forall {
          case (xx, yy) => EA.isEqual(xx, yy)
        }
      }
    )
  implicit def eqOption[A](implicit EA: Equal[A]): Equal[Option[A]] =
    instance((x, y) => (x, y) match {
